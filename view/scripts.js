...
function sortTable(column) {
  ...
  originalData.sort((a, b) => {
    ...
    if (key === 'time') {
      valA = parseTimeToHours(valA);
      valB = parseTimeToHours(valB);
  ...
  // Update header with arrows
  updateHeaderArrows(column);
}

function updateHeaderArrows(column) {
  // Remove arrows from all headers
  const headers = document.querySelectorAll('#practiceTable th');
  ...
  const activeHeader = document.getElementById(`header-${column}`);
  activeHeader.textContent += arrow;
}
